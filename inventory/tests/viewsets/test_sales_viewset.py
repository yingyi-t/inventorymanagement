...
class SalesViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.material_stock1 = MaterialStockFactory(
            store=store, material=material1, current_capacity=20
        )
        self.material_stock2 = MaterialStockFactory(
            store=store, material=material2, current_capacity=20
        )
    ...
    def test_post_sales_valid_list(self):
        ...
        post_data = {
            "sale": [{"product": 1, "quantity": 5}, {"product": 2, "quantity": 5}]
        }
        ...
        self.assertEqual(
            MaterialStock.objects.get(
                material=self.material_stock1.material
            ).current_capacity,
            0,
        )
        self.assertEqual(
            MaterialStock.objects.get(
                material=self.material_stock2.material
            ).current_capacity,
            0,
        )
    ...
    def test_post_sales_no_list_in_sale(self):
        view = views.RestockViewSet.as_view({'post': 'create'})
        post_data = {"sale": {"product": 1, "quantity": 5}}
        request = self.factory.post('/sales/', post_data, format='json')
        force_authenticate(request, user=self.user, token=self.token)
        response = view(request)
    ...
    def test_post_sales_product_id_invalid(self):
        view = views.RestockViewSet.as_view({'post': 'create'})
        post_data = {"sale": [{"product": 5, "quantity": 5}]}
    ...
    def test_post_sales_quantity_exceed(self):
        view = views.RestockViewSet.as_view({'post': 'create'})
        post_data = {
            "sale": [{"product": 1, "quantity": 10}, {"product": 2, "quantity": 10}]
        }
        request = self.factory.post('/sales/', post_data, format='json')
        force_authenticate(request, user=self.user, token=self.token)
        response = view(request)
